...
class _OrderDetailsState extends State<OrderDetails> {
  ...
  fetchOrderDetails() async {
    ...
    if (orderDetailsResponse.detailed_orders.length > 0) {
      ...
      if ((_orderDetails?.links?.details != null) &&
          (orderDetailsResponse.detailed_orders[0].links?.details != null)) {}
    }
    ...
    InfoDialog.show(
      ...
      content: SizedBox(
        child: Column(
          ...
          children: [success, SizedBox(height: 3), failed],
        ),
      ),
    );
  }
  ...
  Widget _buildTrackingContent() {
    final trackingInfo = _orderDetails!.links!.tracking_link!;
    final uri = Uri.tryParse(trackingInfo);
    final isValidUrl = uri != null && (uri.scheme == 'http' || uri.scheme == 'https');

    if (isValidUrl) {
      // It's a valid URL - show as clickable link
      return Row(
        children: [
          Expanded(
            child: Text(
              'Click to track your order online',
              style: TextStyle(
                color: MyTheme.font_grey,
                fontSize: 12,
              ),
      ...
      // It's a tracking code/number - show as copyable text
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
          ...
          Row(
            children: [
              ...
              IconButton(
                onPressed: () {
                  Clipboard.setData(ClipboardData(text: trackingInfo));
                  ToastComponent.showDialog('Tracking code copied to clipboard');
                },
                ...
                SliverList(
                  delegate: SliverChildListDelegate([
                    ...
                    Padding(
                      ...
                      child: Column(
                        children: [
                          if (_orderedItemList.isEmpty && _orderItemsInit)
                            ShimmerHelper().buildBasicShimmer(height: 100.0)
                          else if (_orderedItemList.isNotEmpty)
                          ...
                          if (_orderDetails?.links?.tracking_link != null &&
                              _orderDetails!.links!.tracking_link!.isNotEmpty)
                            Padding(
                              padding: const EdgeInsets.symmetric(
                                horizontal: 0.0,
                                vertical: 10.0,
      ...
      child: Column(
        ...
        children: [
          TimelineTile(
            ...
            afterLineStyle:
                _stepIndex >= 1
                    ? LineStyle(color: Colors.green, thickness: 5)
                    : LineStyle(color: MyTheme.medium_grey, thickness: 4),
          ),
          TimelineTile(
            axis: TimelineAxis.vertical,
            alignment: TimelineAlign.end,
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(Icons.done_all, color: Colors.purple, size: 18),
                  ),
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal: 20.0),
                    child: Container(
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        child: ListView.separated(
          separatorBuilder:
              (context, index) => Divider(color: MyTheme.medium_grey),
